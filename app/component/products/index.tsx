...
export function Index() {
  ...
  useEffect(() => {
    dispatch(
      fetchProducts({
        category: data.categoryFilterSelectedOption,
        sortType: data.sortFilterSelectedOption,
      }),
    );
  }, []);
  ...
  async function sortFilterOnChange(e: ChangeEvent<HTMLSelectElement>) {
    ...
    dispatch(
      fetchProducts({ category: data.categoryFilterSelectedOption, sortType: SortType[value] }),
    );
    setData({
      ...data,
      productsIndexStartFrom: 0,
  ...
  function increment() {
    const isListEnd =
      products.length - (data.productsIndexStartFrom + data.defaultProductsCountPerPage) <= 0;
    setData({
      ...data,
      productsIndexStartFrom: isListEnd
        ? 0
        : data.productsIndexStartFrom + data.defaultProductsCountPerPage,
    });
  }
  function decrement() {
    ...
    <div className="mx-3 ">
      ...
      <div className="flex flex-row justify-between py-3">
        <div className="justify-start flex">
          <Pagination
            startFrom={data.productsIndexStartFrom + 1}
            endAt={
              data.productsIndexStartFrom + data.defaultProductsCountPerPage > products.length
                ? products.length
                : data.productsIndexStartFrom + data.defaultProductsCountPerPage
            }
            ...
            increment={increment}
            decrement={decrement}
            isDisabled={products.length < data.defaultProductsCountPerPage}
          />
        </div>
